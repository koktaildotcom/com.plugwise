...
class PlugwiseAPI {
	...
	fetchData(device) {
		return new Promise((resolve, reject) => {
			...
			request({url: url, method: 'GET', timeout: 4000}, function (error, response, body) {
	...
	discoverDevices(device_type) {
		...
		return new Promise(resolve => {
			...
			var browser = require('mdns-js').createBrowser(mdns.tcp("plugwise"));
			...
			browser.on('ready', () => {
				...
				var success = false;
				browser.on('update', parseDevice.bind(this));

				function parseDevice(data) {

					// Only add devices of desired category
					if (data.txt && data.txt[0].indexOf(device_type) > -1) {
