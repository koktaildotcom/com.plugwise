...
class Anna extends EventEmitter {
		...
		this._limitTemperature(input, function (temperature) {
			request({
				...
				if (error) {
					...
					this.emit("target_temperature", this, temperature);
	...
	_getMeasure(callback) {
		...
		// Make API request
		request({url: this._createAPIUrl(), method: 'GET', headers: {'Content-Type': 'text/xml'}}, function (error, response, body) {
			if (error) {
				...
				if (doc) {
					...
					// Loop over elements in doc
					doc.forEach(function (element) {

						// Check if measured temperature is provided
						if (element.logs && element.logs.point_log) {
							var temp_data = element.logs.point_log.filter(function (x) { return x.type === 'temperature' })[0];
	...
	_updateTemperatureValues() {
		...
		this._getTarget(function (target_temperature) {
		...
		this._getMeasure(function (measure_temperature) {
	...
	_rediscoverDevice(callback) {

		// Debounce rediscovery
		if (this.discoverDebounce) {
			clearTimeout(this.discoverDebounce);
			this.discoverDebounce = null
		}
		...
		this.discoverDebounce = setTimeout(() => {
			...
			console.log("Anna: rediscover device");
			...
			PlugwiseAPI.discoverDevices('smile').then(devices => {
				...
				if (result.hostname == this.hostname && this.ip != result.address[0]) {

					// Update ip address
					this.ip = result.address[0];

					// Emit device available
					if (!this.status) {

						console.log("Anna: rediscovery successful, new ip " + this.ip);
	...
	remove() {
		...
		// Remove interval to prevent memory leak
